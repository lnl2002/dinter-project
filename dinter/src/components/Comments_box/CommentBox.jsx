...
const repliedComments = [
  {
    _id: "65e31fd10abdafe825772432",
    postId: "65e29fbb59cd76ea070cd25d",
    userId: {
      _id: '65e1f0fcdcf9261845e01376',
      username: 'phuongtest',
      avatar: 'public\\images\\users\\image-1709652812360photo-4-16888099584551986437472.png'
    },
    content: "troi oi",
    liked: [],
    createdAt: "2024-03-02T12:47:13.852Z",
    updatedAt: "2024-03-02T12:47:13.852Z"
  },
  {
    _id: "65e31fd10abdafe825772d88",
    postId: "65e29fbb59cd76ea070cd25d",
    userId: {
      _id: '65e1f0fcdcf9261845e01376',
      username: 'phuongtest',
...
function CommentBox({
  ...
  const getRepliedCommentNumber = async () => {
    setRepliedCommentNumber(2);
  }

  const getRepliedComment = async () => {
    setRepliedComment(repliedComments);
  }
...

      {
        repliedCommentNumber && repliedCommentNumber > 0 ?
          <div style={{ marginLeft: '50px' }}>
            <button onClick={() => {getRepliedComment(); setIsShowReply(!isShowReply)}} style={{ background: 'none' }}>
              <div className='d-flex align-items-center' style={{ gap: '10px' }}>
                <div style={{ width: '25px', height: '1px', background: '#8b8b8b' }}></div>
                <p className='text-app-medium'>{isShowReply? 'Hide replies' : 'View replies' }</p>
              </div>
            </button>
            <div style={{ display:  isShowReply ? 'block' : 'none' }}>
              {
                repliedComment && repliedComment.map(cr => <CommentBox style={{ marginTop: '10px' }} comment={cr.content} user={cr.userId}></CommentBox>)
              }
            </div>
          </div> : <></>
      }
    ...
    <div className='d-flex flex-row' style={{ gap: '10px' }}>
      ...
      <div className='d-flex flex-column  justify-content-center' style={{ gap: '5px' }}>
        <div className='d-flex' style={{ gap: '5px' }}>
          <button className='btn-app-func' onClick={() => { window.location.href = ('/profile/' + user._id) }}><p className='text-app-bold'>{user.username}</p></button>
